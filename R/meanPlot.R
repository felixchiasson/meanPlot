...
meanPlot <- function(data, 
    ...
    if (!all(names(unlist(adjustments)) %in% c("purpose","popSize","decorrelation","samplingScheme")))
    ...
    if (is.character(adjustments$popSize))  
            stop("ERROR: popSize should be a positive number or Inf. Exiting...")
    ...
    # 1.4: invalid within-subject factors
